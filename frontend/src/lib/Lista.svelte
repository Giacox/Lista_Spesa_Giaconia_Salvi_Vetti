<script>
  ...
  async function svuotaLista() {
    try {
      const res = await fetch(`${API_BASE}/api/list/svuota`, { method: 'DELETE' });
    ...
    {#each lista as articolo (articolo.name)}
      <div class="card">
        <ArticoloLista
          ...
          on:rimuovi={async () => {
            try {
              const res = await fetch(`${API_BASE}/api/list/remove/${encodeURIComponent(articolo.name)}`, { method: 'DELETE' });
              const data = await res.json();
              if (!res.ok) {
